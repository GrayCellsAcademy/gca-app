...
import {
  LESSON01_TOPICS, generateLesson01Question, gradeLesson01Answer,
} from "./lesson01Questions";

const POINTS = 5;

// - Column arithmetic display -
function ColumnDisplay({ nums, operator }) {
  ...
  return (
    <div style={{
      ...
      {nums.map((n, i) => (
        <div key={i} style={{ display: "flex", alignItems: "center", justifyContent: "flex-end", gap: 8 }}>
          {i === nums.length - 1 ? (
            <span style={{ color: "var(--blue)", fontWeight: 900, fontSize: 28 }}>{operator}</span>
          ) : (
            <span style={{ color: "transparent", fontSize: 28 }}>+</span>
          )}
          <span style={{ whiteSpace: "pre" }}>{pad(n)}</span>
        </div>
      ))}
      {/* Horizontal line */}
      <div style={{
  ...
  return (
    <div style={{ display: "flex", gap: 8, justifyContent: "center", marginTop: 16 }}>
      <input ref={ref} value={val}
        ...
        style={{ textAlign: "center", fontSize: 30, fontFamily: "var(--mono)", fontWeight: 700, padding: "10px", width: 180 }} />
...
function QuestionDisplay({ question, revealCorrect }) {
  ...
  const isAdd = question.type.startsWith("add");
  const operator = isAdd ? "+" : "-";

  return (
    <div style={{ textAlign: "center" }}>
      <ColumnDisplay nums={question.nums} operator={operator} />
      {revealCorrect && (
        <div style={{ marginTop: 14, fontSize: 28, fontWeight: 900, fontFamily: "var(--mono)", color: "var(--green)" }}>
  ...
  return (
    <div style={{ marginBottom: 12 }}>
      <div style={{ display: "flex", justifyContent: "space-between", fontSize: 20, color: "var(--text3)", marginBottom: 4 }}>
        ...
        <span style={{ fontWeight: 700, color, fontSize: 22 }}>{remaining}s</span>
      </div>
...
// - Teacher View -
function TeacherLesson01({ session, sessionId, uid }) {
  ...
  const currentTopic = LESSON01_TOPICS[currentTopicIdx];
  ...
  const handleGenerate = async () => {
    const q = generateLesson01Question(currentTopic.id);
  ...
  const handleReveal = async () => {
    ...
    for (const ans of answers) {
      if (ans.answer !== undefined && gradeLesson01Answer(ans.answer, question))
  ...
  const correctCount = answers.filter(a => gradeLesson01Answer(a.answer, question)).length;
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
            ...
            {session.status === "revealing" && (
              <>
                ...
                {currentTopicIdx < LESSON01_TOPICS.length - 1 && (
                  <button className="btn btn-primary" onClick={() => {
                    const next = currentTopicIdx + 1;
                    setCurrentTopicIdx(next);
                    const q = generateLesson01Question(LESSON01_TOPICS[next].id);
                    const qId = "q_" + Date.now().toString(36);
                    q.id = qId; q.points = POINTS;
                    revealedRef.current = false; setAnswers([]);
                  ...
                  }}>Next: {LESSON01_TOPICS[currentTopicIdx + 1]?.label}</button>
                )}
              </>
            )}
          ...
          {question && (session.status === "question" || session.status === "revealing") && (
            <>
              <div className="card">
                ...
                <div style={{ fontSize: 20, fontWeight: 700, marginBottom: 16 }}>{question.prompt}</div>
                ...
                {session.status === "revealing" && (
                  <div style={{ marginTop: 12, background: "rgba(22,163,74,0.06)", border: "1px solid rgba(22,163,74,0.2)", borderRadius: "var(--radius-sm)", padding: "12px 16px" }}>
                    ...
                    <div style={{ fontSize: 26, fontWeight: 900, color: "var(--green)", fontFamily: "var(--mono)" }}>{question.displayAnswer}</div>
                  </div>
                )}
              ...
              <div className="card">
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 5, maxHeight: 280, overflowY: "auto" }}>
                  {Object.entries(participants).map(([pUid, p]) => {
                    ...
                    const correct = has && gradeLesson01Answer(ans.answer, question);
                    return (
...
// - Student View -
function StudentLesson01({ session, sessionId, uid }) {
  ...
  const handleSubmit = async (inputVal) => {
    ...
    if (!ans) return;
    const correct = gradeLesson01Answer(ans, question);
  ...
  if (session.status === "waiting") return (
    <div style={{ textAlign: "center", padding: "60px 20px" }}>
      ...
      <p style={{ color: "var(--text2)", fontSize: 20 }}>Lesson 1 - Column Addition and Subtraction</p>
    </div>
  );
              ...
              <div style={{ fontSize: 22, fontWeight: 800, color: result.correct ? "var(--green)" : "var(--red)" }}>
                ...
                {!result.correct && question?.displayAnswer && (
                  <div style={{ marginTop: 8, fontSize: 20, color: "var(--green)", fontWeight: 400 }}>
                    Answer: <strong style={{ fontFamily: "var(--mono)" }}>{question.displayAnswer}</strong>
                  </div>
                )}
              </div>
            ) : (
...
// - Session Creator -
function CreateLesson01Session({ user, onCreated }) {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      await setDoc(doc(db, "sessions", sessionId), {
        ...
        joinCode, type: "lesson01", status: "waiting",
  ...
  return (
    <div style={{ maxWidth: 480, margin: "0 auto" }}>
      <div className="card">
        <h2 style={{ fontSize: 22, fontWeight: 800, marginBottom: 4 }}>Lesson 1 - Column Addition and Subtraction</h2>
        <p style={{ color: "var(--text2)", fontSize: 20, marginBottom: 16 }}>Column addition and subtraction with and without carrying/borrowing.</p>
        <div style={{ marginBottom: 12 }}>
...
// - Main Export -
export default function Lesson01Session({ user, onHome }) {
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24, flexWrap: "wrap", gap: 12 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <div style={{ width: 40, height: 40, borderRadius: 12, background: "linear-gradient(135deg,var(--blue),var(--orange))", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22, fontWeight: 800, color: "#fff" }}>L1</div>
            <div>
              <div style={{ fontWeight: 800, fontSize: 22 }}>GCA - Lesson 1</div>
              <div style={{ color: "var(--text3)", fontSize: 20 }}>Column Addition and Subtraction</div>
            </div>
          </div>
          <button className="btn btn-ghost btn-sm" onClick={onHome}>Back to Home</button>
        </div>
        {view === "create" && <CreateLesson01Session user={user} onCreated={(sid) => { setSessionId(sid); setView("session"); }} />}
        {view === "session" && session && (
          user.role === "teacher"
            ? <TeacherLesson01 session={session} sessionId={sessionId} uid={user.id} />
            : <StudentLesson01 session={session} sessionId={sessionId} uid={user.id} />
        )}
        {view === "session" && !session && (
...
export { TeacherLesson01 as Lesson01TeacherView, StudentLesson01 as Lesson01StudentView };
