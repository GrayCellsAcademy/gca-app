...
import {
  LESSON07_TOPICS, generateLesson07Question, gradeLesson07Answer,
...
function QuestionDisplay({ question, revealCorrect }) {
  ...
  if (q.type === "warmup-a" || q.type === "warmup-b") {
  ...
  if (q.type === "signed-ooo" || q.type === "signed-var-expr") {
    return (
      <div style={{ textAlign:"center" }}>
        {q.given && <div style={{ fontSize:20,color:"var(--text2)",marginBottom:8 }}>{q.given}</div>}
...
// Numeric or Undefined
function NumOrUndefinedInput({ onSubmit, submitted }) {
  ...
  return (
    <div>
      <div style={{ fontSize:20,color:"var(--text3)",marginBottom:6 }}>
        Enter simplified expression. Use lowercase letters.
      </div>
      <div style={{ display:"flex",gap:8 }}>
    ...
    return (
      <div style={{ fontSize:20,color:"var(--text2)",marginTop:8 }}>
        {q.isUndefined
          ...
          : "Cube root of a negative number is negative: "+q.answer}
      </div>
    );
  }
  return null;
...
// - Teacher view -
function TeacherLesson07({ session, sessionId, uid }) {
  ...
  const currentTopic = LESSON07_TOPICS[currentTopicIdx];
  ...
  const handleGenerate = async () => {
    const q = generateLesson07Question(currentTopic.id);
  ...
  const handleReveal = async () => {
    ...
    for (const ans of answers) {
      if (ans.answer!==undefined&&gradeLesson07Answer(ans.answer,question)) {
  ...
  const handleNextTopic = async () => {
    const nextIdx = Math.min(currentTopicIdx+1,LESSON07_TOPICS.length-1);
    setCurrentTopicIdx(nextIdx);
    const q = generateLesson07Question(LESSON07_TOPICS[nextIdx].id);
    const qId = "q_"+Date.now().toString(36);
    q.id=qId; q.points=POINTS;
    revealedRef.current=false; setAnswers([]);
    await updateDoc(doc(db,"sessions",sessionId),{
  ...
  const correctCount = answers.filter(a=>gradeLesson07Answer(a.answer,question)).length;
          ...
          <div style={{ display:"flex",alignItems:"center",gap:10,flexWrap:"wrap" }}>
            ...
            {session.status==="revealing"&&(
              <>
                ...
                {currentTopicIdx<LESSON07_TOPICS.length-1&&(
                  <button className="btn btn-primary" onClick={handleNextTopic}>
                    Next: {LESSON07_TOPICS[currentTopicIdx+1].label}
                  </button>
                )}
              </>
            )}
              ...
              <div className="card">
                ...
                <div style={{ display:"flex",flexDirection:"column",gap:5,maxHeight:280,overflowY:"auto" }}>
                  {Object.entries(participants).map(([pUid,p])=>{
                    ...
                    const correct=has&&gradeLesson07Answer(ans.answer,question);
                    return (
...
// - Student view -
function StudentLesson07({ session, sessionId, uid }) {
  useKaTeX();
  ...
  const handleSubmit = async (inputVal) => {
    ...
    if (!ans) return;
    const correct = gradeLesson07Answer(ans,question);
  ...
  if (session.status==="waiting") return (
    <div style={{ textAlign:"center",padding:"60px 20px" }}>
      ...
      <p style={{ color:"var(--text2)",fontSize:20 }}>Lesson 7 - Signed Multiplication, Powers, and Roots</p>
    </div>
  );
...
// - Session creator -
function CreateLesson07Session({ user, onCreated }) {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      await updateDoc(doc(db,"sessions",sessionId),{ type:"lesson07" });
      onCreated(sessionId);
  ...
  return (
    <div style={{ maxWidth:480,margin:"0 auto" }}>
      <div className="card">
        <h2 style={{ fontSize:22,fontWeight:800,marginBottom:4 }}>Lesson 7 - Signed Multiplication, Powers, and Roots</h2>
        <p style={{ color:"var(--text2)",fontSize:20,marginBottom:16 }}>Signs of products/quotients, negative base powers, roots of negatives, signed OoO and variable expressions.</p>
        <div style={{ marginBottom:12 }}>
...
// - Main export -
export default function Lesson07Session({ user, onHome }) {
      ...
      <div style={{ maxWidth:1200,margin:"0 auto" }}>
        <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:24,flexWrap:"wrap",gap:12 }}>
          <div style={{ display:"flex",alignItems:"center",gap:12 }}>
            <div style={{ width:40,height:40,borderRadius:12,background:"linear-gradient(135deg,var(--blue),var(--purple))",display:"flex",alignItems:"center",justifyContent:"center",fontSize:22,fontWeight:800,color:"#fff" }}>L7</div>
            <div>
              <div style={{ fontWeight:800,fontSize:22 }}>GCA - Lesson 7</div>
              <div style={{ color:"var(--text3)",fontSize:20 }}>Signed Multiplication, Powers, and Roots</div>
            </div>
          </div>
          <button className="btn btn-ghost btn-sm" onClick={onHome}>Back to Home</button>
        </div>
        {view==="create"&&<CreateLesson07Session user={user} onCreated={(sid)=>{ setSessionId(sid); setView("session"); }} />}
        {view==="session"&&session&&(
          user.role==="teacher"
            ?<TeacherLesson07 session={session} sessionId={sessionId} uid={user.id} />
            :<StudentLesson07 session={session} sessionId={sessionId} uid={user.id} />
        )}
        {view==="session"&&!session&&(
