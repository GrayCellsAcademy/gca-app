...
const TABLES = [2, 3]; // unlock in order
...
function calcPct(data) {
  if (data.phase === "complete") return 100;
  if (data.phase === "mastery") {
    const ttDone = TABLES.length;
    const masteryTotal = 3 + 2 + 3; // mul(3) + div(2) + area(3)
    const masteryDone = (data.masteryData?.globalStep || 0);
    return Math.round((ttDone / (TABLES.length + masteryTotal) + masteryDone / (TABLES.length + masteryTotal)) * 100);
  }
  const ttIdx = data.ttData?.tableIdx || 0;
  const ttPhase = data.ttData?.tablePhase || 0;
  return Math.round(((ttIdx * 3 + ttPhase) / (TABLES.length * 3)) * 50);
}
...
export default function Lesson03MasteryPlayer({ user, topic, onHome }) {
  ...
  const [phase, setPhase] = useState("times-tables");
  ...
  const save = async (newPhase, newTtData, newMasteryData, done) => {
    const data = { phase: newPhase, ttData: newTtData, masteryData: newMasteryData, completed: done };
    await fbSaveProgress(user.id, topicId, { started: true, completed: done, percentComplete: calcPct(data), data });
    setPhase(newPhase);
    if (newTtData) setTtData(newTtData);
    if (newMasteryData) setMasteryData(newMasteryData);
  ...
  if (completed) return (
    <div style={{ maxWidth: 520, margin: "0 auto", textAlign: "center", animation: "fadeUp 0.4s ease" }}>
      <div className="card">
        ...
        <h2 style={{ fontSize: 24, fontWeight: 800, marginBottom: 8 }}>Lesson 3 Mastery Complete!</h2>
        <p style={{ color: "var(--text2)", fontSize: 15, marginBottom: 24 }}>
          Times tables, column multiplication, long division, and area mastered!
        </p>
      ...
      <div style={{ maxWidth: 700, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20, flexWrap: "wrap", gap: 12 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <div>
              <div style={{ fontWeight: 800, fontSize: 17 }}>Lesson 3 Mastery</div>
              <div style={{ color: "var(--text3)", fontSize: 12 }}>
                {phase === "times-tables" ? "Times Tables" : "Mastery Activities"}
              </div>
            </div>
          </div>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 20 }}>
          {["Times Tables", "Mastery Activities"].map((label, i) => {
            const isActive = (i === 0 && phase === "times-tables") || (i === 1 && phase === "mastery");
            const isDone = i === 0 && phase === "mastery";
            return (
              <div key={i} style={{ flex: 1, padding: "8px 12px", borderRadius: "var(--radius-sm)", textAlign: "center", background: isDone ? "rgba(16,185,129,0.08)" : isActive ? "rgba(232,99,10,0.08)" : "var(--surface)", border: "1px solid " + (isDone ? "rgba(16,185,129,0.3)" : isActive ? "rgba(232,99,10,0.3)" : "var(--border)") }}>
                <div style={{ fontSize: 13, fontWeight: 700, color: isDone ? "var(--green)" : isActive ? "var(--blue)" : "var(--text3)" }}>
                  {isDone ? "Done - " : ""}{label}
                </div>
              </div>
            );
        ...
        {phase === "times-tables" && (
          <TimesTableSection
            ...
            ttData={ttData}
            onComplete={() => save("mastery", ttData, masteryData, false)}
            onSave={(newTtData, done) => save(done ? "mastery" : "times-tables", newTtData, masteryData, false)}
          />
        )}
        {phase === "mastery" && (
          <MasterySection
            ...
            onSave={(newMd) => save("mastery", ttData, newMd, newMd.globalStep >= MASTERY_STEPS.length)}
            onComplete={() => save("mastery", ttData, { globalStep: MASTERY_STEPS.length, streak: 0 }, true)}
          />
        )}
      </div>
    </div>
