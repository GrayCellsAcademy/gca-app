...
const TABLES = [4, 5];
// Review tables from Lesson 3 homework (2s and 3s already mastered)
const REVIEW_TABLES = { 4: [2, 3], 5: [2, 3, 4] };
const SKIP_GOAL = 30;
...
function Stage1({ n, onComplete }) {
  ...
  if (!started && !done) return (
    <div className="card" style={{ maxWidth:480,margin:"0 auto",textAlign:"center" }}>
      ...
      <p style={{ color:"var(--text2)",fontSize:15,marginBottom:8 }}>Skip count from {n} to {n*10}, starting at {n}.</p>
      <p style={{ color:"var(--text3)",fontSize:13,marginBottom:20 }}>Beat {SKIP_GOAL} seconds to advance. Timer starts when you click Start.</p>
  ...
  if (done) {
    ...
    return (
      <div className="card" style={{ maxWidth:480,margin:"0 auto",textAlign:"center",animation:"popIn 0.3s ease" }}>
        ...
        <h3 style={{ fontSize:20,fontWeight:800,marginBottom:8 }}>{beat ? "Goal beaten! Advancing..." : "Good try! Goal is " + SKIP_GOAL + "s"}</h3>
        {bestTime !== null && <p style={{ color:"var(--text3)",fontSize:13,marginBottom:16 }}>Best this session: {bestTime}s</p>}
        {beat
          ? <button className="btn btn-success btn-lg" style={{ width:"100%" }} onClick={() => onComplete(elapsed)}>On to Stage 2!</button>
  ...
  return (
    <div style={{ maxWidth:480,margin:"0 auto" }}>
      ...
      <div style={{ textAlign:"center",marginBottom:12 }}>
        ...
        <div style={{ fontSize:13,color:"var(--text3)" }}>Goal: {SKIP_GOAL}s{bestTime!==null?" | Best: "+bestTime+"s":""}</div>
      </div>
      <div className="card" style={{ textAlign:"center" }}>
        <div style={{ fontSize:16,color:"var(--text2)",marginBottom:16 }}>Skip count by {n}s - type each number in order</div>
        <div style={{ fontFamily:"var(--mono)",fontSize:38,fontWeight:900,color:wrongFlash?"var(--red)":"var(--amber)",marginBottom:20,transition:"color 0.15s" }}>?</div>
...
function Stage2({ n, onComplete }) {
  ...
  const handleSubmit = () => {
    ...
    if (val === currentQ.answer) {
      if (qIdx === 9) {
        ...
        if (newPasses >= STAGE2_PASSES) { setDone(true); }
        else { setQIdx(0); }
      } else { setQIdx(i => i + 1); }
  ...
  if (intro) return (
    <div className="card" style={{ maxWidth:480,margin:"0 auto",textAlign:"center" }}>
      ...
      <div style={{ display:"flex",flexDirection:"column",gap:8,marginBottom:20,textAlign:"left",background:"var(--bg2)",borderRadius:"var(--radius-sm)",padding:"14px 16px" }}>
        <div style={{ fontSize:15,color:"var(--text2)" }}>Questions in order: {n}x1 through {n}x10</div>
        ...
        <div style={{ fontSize:13,color:"var(--text3)" }}>Wrong answer resets back to question 1</div>
      </div>
  ...
  if (done) return (
    <div className="card" style={{ maxWidth:480,margin:"0 auto",textAlign:"center",animation:"popIn 0.3s ease" }}>
      <div style={{ fontSize:48,fontWeight:900,color:"var(--green)",marginBottom:8 }}>3 Passes!</div>
      <p style={{ color:"var(--text2)",fontSize:15,marginBottom:24 }}>Perfect answers for all {n}x1 through {n}x10, three times in a row!</p>
  ...
  return (
    <div style={{ maxWidth:480,margin:"0 auto" }}>
      ...
      <div className="card" style={{ textAlign:"center" }}>
        ...
        <div style={{ fontFamily:"var(--mono)",fontSize:52,fontWeight:900,color:"var(--text)",marginBottom:20,letterSpacing:"-1px" }}>{n} x {currentQ.b} = ?</div>
        <input ref={inputRef} value={input} onChange={e=>setInput(e.target.value.replace(/\D/g,""))}
          onKeyDown={e=>e.key==="Enter"&&handleSubmit()} inputMode="numeric" placeholder="?"
...
function buildStage3Questions(n, reviewTables) {
  const current = Array.from({ length:10 },(_,i) => ({ n,b:i+1,answer:n*(i+1),streakNeeded:STAGE3_STREAK,streak:0,isCurrent:true }));
  const review = reviewTables.flatMap(t => Array.from({ length:10 },(_,i) => ({ n:t,b:i+1,answer:t*(i+1),streakNeeded:STAGE3_REVIEW,streak:0,isCurrent:false })));
...
function Stage3({ n, reviewTables, onComplete }) {
  const [intro, setIntro] = useState(true);
  const [done, setDone] = useState(false);
  const [questions, setQuestions] = useState(() => buildStage3Questions(n, reviewTables));
  const [qIdx, setQIdx] = useState(0);
  const [input, setInput] = useState("");
  const [wrongPanel, setWrongPanel] = useState(null);
  ...
  if (intro) return (
    <div className="card" style={{ maxWidth:480,margin:"0 auto",textAlign:"center" }}>
      ...
      <div style={{ display:"flex",flexDirection:"column",gap:8,marginBottom:20,textAlign:"left",background:"var(--bg2)",borderRadius:"var(--radius-sm)",padding:"14px 16px" }}>
        ...
        <div style={{ fontSize:15,color:"var(--text2)" }}>Each needs <strong>3 correct in a row</strong></div>
        <div style={{ fontSize:15,color:"var(--red)",fontWeight:700 }}>10 seconds per question!</div>
        <div style={{ fontSize:13,color:"var(--text3)" }}>Includes review: x{reviewTables.join(", x")}</div>
      </div>
  ...
  if (done) return (
    <div className="card" style={{ maxWidth:480,margin:"0 auto",textAlign:"center",animation:"popIn 0.3s ease" }}>
      <div style={{ fontSize:48,fontWeight:900,color:"var(--green)",marginBottom:8 }}>Table Mastered!</div>
      <p style={{ color:"var(--text2)",fontSize:15,marginBottom:24 }}>You cleared all questions in the {n}s table!</p>
  ...
  return (
    <div style={{ maxWidth:480,margin:"0 auto" }}>
      ...
      <div className="card" style={{ textAlign:"center" }}>
        ...
        {!currentQ.isCurrent && <div style={{ fontSize:11,color:"var(--text3)",marginBottom:6,textTransform:"uppercase",letterSpacing:"0.06em" }}>Review - x{currentQ.n}</div>}
        <div style={{ fontFamily:"var(--mono)",fontSize:52,fontWeight:900,color:"var(--text)",marginBottom:12,letterSpacing:"-1px" }}>{currentQ.n} x {currentQ.b} = ?</div>
...
function TableSession({ n, onComplete }) {
  const [stage, setStage] = useState(1);
  const reviewTables = REVIEW_TABLES[n] || [];
  return (
    <div>
  ...
  if (completed) return (
    <div style={{ maxWidth:520,margin:"0 auto",textAlign:"center",animation:"fadeUp 0.4s ease" }}>
      <div className="card">
        ...
        <p style={{ color:"var(--text2)",fontSize:15,marginBottom:24 }}>You have mastered the 4s and 5s times tables!</p>
      ...
      <div style={{ maxWidth:600,margin:"0 auto" }}>
        <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:20,flexWrap:"wrap",gap:12 }}>
          <div style={{ display:"flex",alignItems:"center",gap:12 }}>
            ...
            <div>
              <div style={{ fontWeight:800,fontSize:17 }}>Times Table (4 and 5)</div>
