        ...
        {items.map((item, i) => (
          <div key={i} style={{ background: "var(--bg2)", borderRadius: "var(--radius-sm)", padding: "10px 14px" }}>
            ...
            <input value={answers[i]} onChange={e => setAnswers(prev => prev.map((x, j) => j === i ? e.target.value : x))}
              disabled={submitted} placeholder={placeholder || "e.g. 1/3"}
...
function StepInput({ question, onSubmit, submitted, stages }) {
  ...
  const handleNext = () => {
    if (feedback?.correct && stage < stages.length - 1) {
      setStage(s => s + 1); setFeedback(null); setInput(""); setTimeout(() => ref.current?.focus(), 80);
    } else if (feedback?.correct && stage === stages.length - 1) {
      onSubmit(input.trim());
    } else {
      setFeedback(null); setInput(""); setTimeout(() => ref.current?.focus(), 80);
    }
  };
  ...
  return (
    <div>
      ...
      <div style={{ background: "rgba(27,143,255,0.07)", border: "1px solid rgba(27,143,255,0.2)", borderRadius: "var(--radius-sm)", padding: "8px 12px", marginBottom: 10 }}>
        ...
        {stages[stage]?.hint && <div style={{ fontSize: 17, color: "var(--text2)", marginTop: 3 }}>{stages[stage].hint}</div>}
      </div>
      {feedback ? (
        <div style={{ textAlign: "center" }}>
          ...
          {!feedback.correct && (
            <div style={{ fontSize: 19, color: "var(--text2)", marginBottom: 10, fontFamily: "var(--mono)" }}>
...
function gradeAnswer(input, question) {
  ...
  try { return gradeLesson21Answer(input, question); } catch { return false; }
}
...
function QuestionDisplay({ question: q, revealCorrect }) {
  ...
  if (q.type === "warmup-a") return (
    <div style={{ textAlign: "center" }}>
      <div style={{ fontFamily: "var(--mono)", fontSize: 26, fontWeight: 700, marginBottom: 6 }}>18:24</div>
      {revealCorrect && <div style={{ color: "var(--green)", fontWeight: 800, fontSize: 22 }}>{q.displayAnswer}</div>}
    </div>
  );
  if (q.type === "warmup-b") return (
    <div style={{ textAlign: "center" }}>
      <div style={{ fontFamily: "var(--mono)", fontSize: 24, fontWeight: 700, marginBottom: 6 }}>{q.ratio}</div>
  ...
  if (q.type === "warmup-d") return (
    <div style={{ textAlign: "center" }}>
      <div style={{ fontSize: 19, color: "var(--text2)", marginBottom: 8 }}>{q.stmt}</div>
      {revealCorrect && <div style={{ color: "var(--green)", fontWeight: 800, fontSize: 22, fontFamily: "var(--mono)" }}>{q.displayAnswer}</div>}
    </div>
  );

  // Step types: show the decimal
  if (STEP_TYPES.includes(q.type)) return (
    <div style={{ textAlign: "center" }}>
      <KaTeX expr={q.overline} block />
      {revealCorrect && <div style={{ color: "var(--green)", fontWeight: 800, fontSize: 22, fontFamily: "var(--mono)", marginTop: 6 }}>
        <KaTeX expr={`\\dfrac{${q.rn}}{${q.rd}}`} />
      </div>}
    </div>
  );

  // Multi types: show on reveal
  if (!revealCorrect) return null;
  const items = q.problems || [];
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
      {items.map((item, i) => (
        <div key={i} style={{ background: "var(--bg2)", borderRadius: "var(--radius-sm)", padding: "8px 14px", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8 }}>
          <KaTeX expr={item.overline} />
          <span style={{ color: "var(--green)", fontWeight: 700, fontSize: 19 }}>
            <KaTeX expr={`\\dfrac{${item.rn}}{${item.rd}}`} />
          </span>
        </div>
      ))}
    </div>
...
function AnswerInput({ question, onSubmit, submitted }) {
  ...
  if (t === "warmup-a") return <TextInput onSubmit={onSubmit} submitted={submitted} placeholder="e.g. 3:4" />;
  if (t === "warmup-b") return <TextInput onSubmit={onSubmit} submitted={submitted} placeholder="x = ?" />;
  if (t === "warmup-c") return <TextInput onSubmit={onSubmit} submitted={submitted} placeholder="e.g. 0.333..." />;
  if (t === "warmup-d") return <TextInput onSubmit={onSubmit} submitted={submitted} placeholder="e.g. 2:5" />;

  if (t === "single-rep-direct")
...
function StudentReveal({ result, question }) {
  ...
  const isMulti = MULTI_TYPES.includes(question?.type);
  const graderMap = {
    "single-rep-direct": gradeSingleRepDirectItem,
    "two-rep-direct": gradeTwoRepDirectItem,
    "mixed-rep-direct": gradeMixedRepDirectItem,
  };
  return (
    <div>
...
// -- Teacher --
function TeacherLesson21({ session, sessionId }) {
  ...
  const handleGenerate = async (tIdx) => {
    ...
    try {
      const q = generateLesson21Question(LESSON21_TOPICS[idx].id);
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
            ...
            {session.status === "revealing" && (
              <>
                ...
                {topicIdx < LESSON21_TOPICS.length - 1 && (
                  <button className="btn btn-primary" disabled={session.status === "question"}
                    ...
                    Next: {LESSON21_TOPICS[topicIdx + 1]?.label}
                  </button>
                )}
              </>
            )}
            ...
            <>
              <div className="card">
                <div style={{ fontSize: 20, color: "var(--text3)", marginBottom: 8 }}>
                  {LESSON21_TOPICS[topicIdx]?.label} - {submittedCount}/{totalStudents} submitted - {correctCount} correct
                </div>
...
// -- Student --
function StudentLesson21({ session, sessionId, uid }) {
  ...
  if (session.status === "waiting") return (
    <div style={{ textAlign: "center", padding: "60px 20px" }}>
      ...
      <p style={{ color: "var(--text2)", fontSize: 20 }}>Lesson 21 - Converting Repeating Decimals to Fractions</p>
    </div>
  );
...
// -- Session Creator --
function CreateLesson21Session({ user, onCreated }) {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      await setDoc(doc(db, "sessions", sessionId), {
        ...
        joinCode, type: "lesson21", status: "waiting",
  ...
  return (
    <div style={{ maxWidth: 480, margin: "0 auto" }}>
      <div className="card">
        <h2 style={{ fontSize: 22, fontWeight: 800, marginBottom: 4 }}>Lesson 21 - Repeating Decimals to Fractions</h2>
        <p style={{ color: "var(--text2)", fontSize: 20, marginBottom: 16 }}>Single-digit, two-digit, and mixed repeating decimal conversions.</p>
        <div style={{ marginBottom: 12 }}>
...
export default function Lesson21Session({ user, onHome }) {
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24, flexWrap: "wrap", gap: 12 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <div style={{ width: 40, height: 40, borderRadius: 12, background: "linear-gradient(135deg,var(--blue),var(--orange))", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22, fontWeight: 800, color: "#fff" }}>L21</div>
            <div>
              <div style={{ fontWeight: 800, fontSize: 22 }}>GCA - Lesson 21</div>
              <div style={{ color: "var(--text3)", fontSize: 20 }}>Converting Repeating Decimals to Fractions</div>
            </div>
          </div>
          <button className="btn btn-ghost btn-sm" onClick={onHome}>Back to Home</button>
        </div>
        {view === "create" && <CreateLesson21Session user={user} onCreated={(sid) => { setSessionId(sid); setView("session"); }} />}
        {view === "session" && session && (
          user.role === "teacher"
            ? <TeacherLesson21 session={session} sessionId={sessionId} uid={user.id} />
            : <StudentLesson21 session={session} sessionId={sessionId} uid={user.id} />
        )}
...
export { TeacherLesson21 as Lesson21TeacherView, StudentLesson21 as Lesson21StudentView };
