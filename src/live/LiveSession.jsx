...
// ΓöÇΓöÇΓöÇ Helpers ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
function medalEmoji(rank) {
  if (rank === 1) return "≡ƒÑç";
  if (rank === 2) return "≡ƒÑê";
  if (rank === 3) return "≡ƒÑë";
  return `#${rank}`;
}

function pct(score) { return Math.round((score / TOTAL_POINTS) * 100); }

// ΓöÇΓöÇΓöÇ Math Display ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  ...
  return (
    <div style={style}>
      <div style={{ marginBottom: 8, fontSize: "clamp(14px,2.5vw,18px)", color: "var(--text2)", fontWeight: 500 }}>
        [{question.section}] {question.sectionTitle} ΓÇö {question.points} pts
      </div>
...
// ΓöÇΓöÇΓöÇ Timer Bar ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇΓöÇ Leaderboard ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  ...
  return (
    <div style={{ maxWidth: 600, margin: "0 auto" }}>
      <h2 style={{ fontSize: 24, fontWeight: 800, marginBottom: 4, textAlign: "center" }}>
        {isEnded ? "≡ƒÅå Final Scores" : "≡ƒôè Leaderboard"}
      </h2>
...
// ΓöÇΓöÇΓöÇ Teacher View ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
            ...
            {session.status === "question" && (
              <button className="btn btn-ghost" onClick={handleReveal}>≡ƒôè Reveal Answers</button>
            )}
            {session.status === "revealing" && (
              <button className="btn btn-primary" onClick={handleNext}>
                {qIdx + 1 >= REVIEW_QUESTIONS.length ? "End Session" : `Next Question ΓåÆ`}
              </button>
            )}
                      ...
                      {ans ? (
                        <>
                          ...
                          <span style={{ fontSize: 14, fontWeight: 700, color: ans.correct ? "var(--green)" : "var(--red)" }}>
                            {ans.correct ? `+${ans.points}` : "Γ£ù"}
                          </span>
                        </>
                      ) : (
                        <span style={{ fontSize: 12, color: "var(--text3)" }}>waitingΓÇª</span>
                      )}
                    </div>
                  </div>
                );
...
// ΓöÇΓöÇΓöÇ Student View ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  ...
  if (session.status === "waiting") {
    return (
      <div style={{ textAlign: "center", padding: "60px 20px" }}>
        <div style={{ fontSize: 48, marginBottom: 16 }}>ΓÅ│</div>
        <h2 style={{ fontSize: 24, fontWeight: 800, marginBottom: 8 }}>Waiting for the teacher to startΓÇª</h2>
        <p style={{ color: "var(--text2)", fontSize: 15 }}>
          You're in! Get ready ΓÇö the review is about to begin.
        </p>
    ...
    return (
      <div className="card" style={{ maxWidth: 700, margin: "0 auto" }}>
        <div style={{ textAlign: "center", marginBottom: 24 }}>
          <div style={{ fontSize: 48, marginBottom: 8 }}>≡ƒÄë</div>
        ...
        <div style={{ marginTop: 20 }}>
          ...
          {session.status === "revealing" ? (
            <div style={{ animation: "fadeUp 0.3s ease" }}>
              {result ? (
                <div style={{ textAlign: "center" }}>
                  <div style={{ fontSize: 48, marginBottom: 8 }}>{result.correct ? "≡ƒÄë" : "≡ƒÿö"}</div>
                ...
                <div style={{ textAlign: "center", color: "var(--text3)" }}>
                  <div style={{ fontSize: 32, marginBottom: 8 }}>ΓÅ░</div>
            ...
            <div style={{ textAlign: "center", animation: "popIn 0.3s ease" }}>
              <div style={{ fontSize: 48, marginBottom: 8 }}>Γ£à</div>
              <div style={{ fontSize: 18, fontWeight: 700, color: "var(--green)", marginBottom: 4 }}>Answer submitted!</div>
              <div style={{ fontSize: 14, color: "var(--text2)" }}>
                Waiting for other students and the teacher to revealΓÇª
              </div>
            </div>
          ) : (
            <>
...
// ΓöÇΓöÇΓöÇ Join Screen ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
function JoinScreen({ user, onJoined }) {
  ...
  return (
    <div style={{ maxWidth: 420, margin: "0 auto", textAlign: "center" }}>
      <div className="card">
        <div style={{ fontSize: 48, marginBottom: 12 }}>≡ƒÄ«</div>
        ...
        <button className="btn btn-primary btn-lg" style={{ width: "100%" }}
          ...
          {loading ? "JoiningΓÇª" : "Join Session ΓåÆ"}
        </button>
      </div>
    </div>
  );
...
// ΓöÇΓöÇΓöÇ Teacher Create Screen ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  ...
  return (
    <div style={{ maxWidth: 480, margin: "0 auto" }}>
      <div className="card">
        ...
        <p style={{ color: "var(--text2)", fontSize: 14, marginBottom: 20 }}>
          This review covers fractions, fraction operations, and equations ΓÇö mirroring Test 2.
        </p>
        <div style={{ marginBottom: 14 }}>
          ...
          <select value={selectedClass} onChange={e => setSelectedClass(e.target.value)} style={{ width: "100%", padding: "10px 12px", fontSize: 14 }}>
            <option value="">Select a classΓÇª</option>
        ...
        <div style={{ background: "var(--bg2)", borderRadius: "var(--radius-sm)", padding: "12px 14px", marginBottom: 20, fontSize: 13, color: "var(--text2)" }}>
          <strong>{REVIEW_QUESTIONS.length} questions</strong> ΓÇö {TOTAL_POINTS} points total<br />
          Sections: Equivalent Fractions ┬╖ Simplify ┬╖ Operations ┬╖ Equations
        </div>
        <button className="btn btn-primary btn-lg" style={{ width: "100%" }}
          onClick={handleCreate} disabled={loading || !selectedClass}>
          {loading ? "CreatingΓÇª" : "Create Session ≡ƒÜÇ"}
        </button>
      </div>
    </div>
  );
...
// ΓöÇΓöÇΓöÇ Main LiveSession Component ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 32, flexWrap: "wrap", gap: 12 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <div style={{ width: 40, height: 40, borderRadius: 12, background: "linear-gradient(135deg,var(--blue),var(--cyan))", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20 }}>≡ƒÄô</div>
            <div>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            {view !== "menu" && !sessionId && (
              <button className="btn btn-ghost btn-sm" onClick={() => setView("menu")}>ΓåÉ Back</button>
            )}
            <button className="btn btn-ghost btn-sm" onClick={onHome}>ΓåÉ Home</button>
          </div>
        </div>
