      ...
      {svm.map((m, i) => {
        ...
        return (
          <g key={i}>
            ...
            <text x={m.x + perpX} y={m.y + perpY + 7} textAnchor="middle" fontSize="20" fontWeight="700" fill="var(--text)" fontFamily="var(--mono)">{segments[i]} {unit}</text>
          </g>
        );
      })}
    ...
    <svg viewBox={"0 0 " + W + " " + H} style={{ width: "100%", maxWidth: 320, display: "block", margin: "0 auto" }}>
      ...
      {mids.map((m, i) => (
        <g key={i}>
          ...
          <text x={m.x} y={m.y + 6} textAnchor="middle" fontSize="20" fontWeight="700" fill="var(--text)" fontFamily="var(--mono)">{lengths[i]}{unit}</text>
        </g>
      ))}
          ...
          <g key={i} style={{ cursor: mode === "equal-sides" ? "pointer" : "default" }}
            ...
            {showLabel(i) && (
              <g>
                ...
                <text x={s.mx} y={s.my + 7} textAnchor="middle" fontSize="20" fontWeight="700" fill={isRevealed(i) ? "var(--green)" : "var(--text)"} fontFamily="var(--mono)">{s.label}{unit}</text>
              </g>
            )}
          </g>
        );
      ...
      {labels.map((lbl, i) => {
        ...
        return (
          <g key={i}>
            ...
            <text x={lbl.x} y={lbl.y + 5} textAnchor="middle" fontSize="20" fontWeight="700"
...
function ColumnAdditionReveal({ numbers, label }) {
  ...
  return (
    <div style={{ marginTop: 10, display: "inline-block" }}>
      {label && <div style={{ fontSize: 20, color: "var(--text3)", marginBottom: 6, fontWeight: 600 }}>{label}</div>}
...
function MissingSideCalc({ sideIdx, allSides, unit, hideIndices }) {
  ...
  // Check if ANY other same-dir side (known or hidden) has the same length -> congruent pair
  const congruentSide = sameDirSides.find(s => s.i !== sideIdx && s.length === side.length);

  if (congruentSide) {
    // Congruent pair - equal to opposite side, no calculation needed
    return (
      <div style={{ marginTop: 10, padding: "10px 14px", background: "var(--bg3)", borderRadius: "var(--radius-sm)", border: "1px solid var(--green)", display: "inline-block" }}>
        <div style={{ fontSize: 20, color: "var(--text3)", marginBottom: 4, fontWeight: 600 }}>Equal to opposite side</div>
        <div style={{ fontFamily: "var(--mono)", fontSize: 24, fontWeight: 800, color: "var(--green)" }}>{side.length}{unit}</div>
      </div>
    );
  } else if (isLong) {
    // Long side = sum of shorter parallel sides (excluding all hidden sides)
    ...
    // Short side = long side minus other known short sides
    ...
    return (
      <div style={{ display: "flex", gap: 16, flexWrap: "wrap", marginTop: 8 }}>
        {question.missingAnswers.map((ma, i) => (
          <div key={i}>
            <div style={{ fontSize: 20, fontWeight: 700, color: "var(--text3)", marginBottom: 4 }}>Missing side {i + 1}:</div>
  ...
  if (question.type === "rectilinear-5C" && question.hideIndices && question.sides) {
    ...
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: 12, marginTop: 8 }}>
        ...
        <div>
          <div style={{ fontSize: 20, fontWeight: 700, color: "var(--text3)", marginBottom: 4 }}>Perimeter (all sides):</div>
...
function AnswerInput({ question, onSubmit, submitted, selectedSides, onSideClick, activeMissingIdx }) {
  ...
  return (
    <div>
      <div style={{ fontSize: 20, color: "var(--text3)", marginBottom: 6 }}>Include units (e.g. 45cm, 120ft)</div>
  ...
  return (
    <div style={{ maxWidth: 580, margin: "0 auto" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
        <div style={{ fontSize: 20, color: "var(--text3)" }}>{question ? (LESSON02_TOPICS.find(t => t.id === question.topicId)?.label || "") : ""}</div>
          ...
          <div style={{ textAlign: "center", marginTop: 12 }}>
            {result ? (
              ...
              <div>
                ...
                {question?.displayAnswer && <div style={{ color: "var(--green)", fontSize: 20, marginBottom: 8 }}>Correct: <strong style={{ fontFamily: "var(--mono)" }}>{question.displayAnswer}</strong></div>}
                <RevealCalculation question={question} />
              </div>
            )}
          </div>
  ...
  return (
    <div style={{ maxWidth: 480, margin: "0 auto" }}>
      <div className="card">
        ...
        <p style={{ color: "var(--text2)", fontSize: 20, marginBottom: 16 }}>Line segments, polygon perimeters, rectangles, squares, and composite rectilinear shapes.</p>
        <div style={{ marginBottom: 12 }}>
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24, flexWrap: "wrap", gap: 12 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <div style={{ width: 40, height: 40, borderRadius: 12, background: "linear-gradient(135deg,var(--blue),var(--cyan))", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20, fontWeight: 800, color: "#fff" }}>L2</div>
            <div>
              <div style={{ fontWeight: 800, fontSize: 20 }}>GCA</div>
              <div style={{ color: "var(--text3)", fontSize: 20 }}>Lesson 2 - Geometry</div>
            </div>
          </div>
